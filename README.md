---

<div align="center">
  <h1>Hi! I'm <strong>S4ndulos</strong> 👨‍💻</h1>
  <p><em>Technical Informatics Student | Buenos Aires, Argentina 🇦🇷</em></p>
  <img src="https://komarev.com/ghpvc/?username=S4ndulos&label=Profile%20Views&color=0e75b6&style=flat" alt="Profile Views" />
</div>

---

### About Me
-  **Age:** 16 years old.
-  **Coding:** I'm fluent in **C/C++** and **Python** (low-level systems are my thing), and I'm currently learning **C#** and relational databases.
-  **Networking & Security:** I spend my free time diving into ethical hacking and computer networking.
-  **Current Focus:** Studying **Technical Informatics** and tinkering with **C++** and **C# with .NET 9 Framework**.
-  **Fun Fact:** I love building personal projects and customizing my Arch Linux setup.

---

### 🛠️ Tools & Environments

| OS / Distros                          | Editors                                  | Terminals           |
| ------------------------------------- | ---------------------------------------- | ------------------- |
|  Windows 11/10/7                         | 💻 VSCode / Visual Studio Community      | ⚡ CMD / PowerShell |
|  Arch Linux                         | 📟 Linux Terminal                        | 📦 pacman / npm     |
|  Kali Linux                         | 🖥️ Neovim                                | 🖥️ Alacritty        |
|  Raspberry Pi 5 / PiOS              | ---                                      | ---                 |
|  BlackArch                          | ---                                      | ---                 |
|  Debian                             | ---                                      | ---                 |
|  Ubuntu Server                      | ---                                      | ---                 |

---

### Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/XAMPP-FB7A24?style=for-the-badge&logo=xampp&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
</p>

---

### Featured Project

> **[DesktopManagerStock](https://github.com/MauroKpoxD/DesktopManagerStock)** – *(Description coming from you)* Briefly describe what this project does: e.g., "A desktop application for managing stock inventory, built with [Language] and [Technology]."
>
> 🔗 [Repository Link](https://github.com/MauroKpoxD/DesktopManagerStock)

---

### Connect with Me

<p align="center">
  <a href="https://twitter.com/MauroKpoxD"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" /></a>
  <a href="https://discord.com/users/mauro_kpoxd"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" /></a>
  <a href="mailto:sandulientos827@protonmail.com"><img src="https://img.shields.io/badge/ProtonMail-8B89CC?style=for-the-badge&logo=protonmail&logoColor=white" /></a>
</p>

---

<div align="center">
  <p><i>Thanks for visiting my GitHub profile!</i></p>
</div>
